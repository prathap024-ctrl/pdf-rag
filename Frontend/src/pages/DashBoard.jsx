...
const PDFDashboard = () => {
    ...
    const uploadPromises = pdfFiles.map(async (file) => {
      ...
      try {
        const response = await axios.post(
          `https://pdf-rag-627w.onrender.com/api/pdf/load-pdf`,
          formData,
          {
            headers: {
  ...
  const handleFetchPdf = async () => {
    ...
    try {
      const response = await axios.get(
        `https://pdf-rag-627w.onrender.com/api/pdf/fetch-pdf`
      );
  ...
  const deletePDF = async (id) => {
    try {
      await axios.delete(
        `https://pdf-rag-627w.onrender.com/api/pdf/delete-pdf/${id}`
      );
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post(
        `https://pdf-rag-627w.onrender.com/api/pdf/pdf-response`,
        {
          userQuestion,
